...
function AnimatedName() {
  ...
  const palette = ['#6366f1','#818cf8','#a855f7','#c084fc','#ec4899','#6366f1'];
  return (
    <span className="inline-flex">
      {letters.map((ch, i) => (
        <motion.span key={i}
          animate={{ color:[palette[i], palette[(i+2)%palette.length], palette[(i+4)%palette.length], palette[i]] }}
...
/* ── Skills mini ── */
const catColor = {
  Frontend:'#6366f1', Backend:'#22d3a0', Tools:'#f59e0b', 'Soft Skills':'#a855f7',
};
function SkillsMini() {
      ...
      {Object.entries(skills).map(([cat, items], ci) => (
        <motion.div key={cat}
          ...
          <div className="flex items-center gap-2 mb-3">
            <div className="w-1 h-6 rounded-full" style={{ background:catColor[cat] }} />
            <span className="font-display font-bold text-sm" style={{ color:catColor[cat] }}>{cat}</span>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {items.map((s,si) => (
              <motion.span key={s}
                ...
                style={{ background:`${catColor[cat]}12`, color:catColor[cat], border:`1px solid ${catColor[cat]}30` }}>
                {s}
              </motion.span>
            ))}
          </div>
        ...
        <motion.div key={job.id}
          ...
          <div className="absolute left-0 top-0 w-9 h-9 rounded-full flex items-center justify-center"
               style={{ background:'var(--bg-card)', border:'2px solid var(--accent)',
                        color:'var(--accent)', boxShadow:'0 0 14px rgba(99,102,241,0.28)' }}>
          ...
          <div className="p-5 rounded-2xl"
            ...
            <div className="flex flex-wrap items-start justify-between gap-2 mb-2">
              <div>
                <h4 className="font-display font-bold text-sm" style={{ color:'var(--text-primary)' }}>{job.role}</h4>
                <p className="font-body text-xs mt-0.5" style={{ color:'var(--accent)' }}>{job.company}</p>
              </div>
              <div className="flex flex-col items-end gap-1">
                <span className="text-xs font-body flex items-center gap-1" style={{ color:'var(--text-muted)' }}>
                  <CalendarMonthIcon sx={{ fontSize:11 }} />{job.duration}
                </span>
            ...
            <ul className="space-y-1.5 mt-3">
              {job.highlights.slice(0,3).map((h,j) => (
                <li key={j} className="flex items-start gap-2 text-xs font-body"
                    style={{ color:'var(--text-secondary)' }}>
                  <CheckCircleOutlineIcon sx={{ fontSize:13, color:'var(--accent)', flexShrink:0, marginTop:'1px' }} />
                  {h}
                </li>
              ))}
            </ul>
    ...
    <div ref={ref} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
      {projects.map((p, i) => (
        <motion.div key={p.id}
          ...
          onMouseEnter={e=>{ e.currentTarget.style.borderColor=`${p.accent}50`; e.currentTarget.style.boxShadow=`0 16px 48px ${p.accent}22`; }}
          onMouseLeave={e=>{ e.currentTarget.style.borderColor='var(--border)'; e.currentTarget.style.boxShadow='none'; }}>
          <div className="h-[3px]" style={{ background:`linear-gradient(to right,${p.accent},transparent)` }} />
          <div className="p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              {p.featured && <StarIcon sx={{ fontSize:13, color:'#f59e0b' }} />}
            </div>
            <h4 className="font-display font-bold text-sm mb-1.5" style={{ color:'var(--text-primary)' }}>{p.title}</h4>
            ...
            <div className="flex flex-wrap gap-1 mb-3">
              {p.tech.slice(0,3).map(t => (
                <span key={t} className="text-xs font-body px-1.5 py-0.5 rounded"
                      style={{ background:'var(--bg-secondary)', color:'var(--text-muted)' }}>{t}</span>
              ))}
              {p.tech.length > 3 && <span className="text-xs" style={{ color:'var(--text-muted)' }}>+{p.tech.length-3}</span>}
            </div>
    ...
    <div ref={ref} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
      {services.map((s, i) => (
        <motion.div key={i}
          ...
          style={{ background:'var(--bg-card)', border:'1px solid var(--border)',
                   transition:'all 0.3s ease' }}
          onMouseEnter={e=>{ e.currentTarget.style.borderColor=`${s.accent}45`; e.currentTarget.style.boxShadow=`0 12px 40px ${s.accent}18`; }}
          onMouseLeave={e=>{ e.currentTarget.style.borderColor='var(--border)'; e.currentTarget.style.boxShadow='none'; }}>
          {/* icon */}
          ...
          {/* bottom accent */}
          <div className="mt-4 h-0.5 w-0 rounded-full group-hover:w-full transition-all duration-500"
    ...
    <div ref={ref} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
      {recentWork.map((p, i) => (
        <motion.div key={p.id}
          ...
          style={{ background:'var(--bg-card)', border:`1px solid ${p.accent}30` }}>

          {/* shimmer top bar */}
          <div className="h-[3px] shimmer w-full"
               style={{ background:`linear-gradient(90deg,${p.accent}30,${p.accent},${p.accent}30)` }} />
          ...
          <div className="p-6">
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-body px-2.5 py-1 rounded-full font-semibold"
                      style={{ background:`${p.accent}15`, color:p.accent, border:`1px solid ${p.accent}30` }}>
                  {p.category}
                </span>
              </div>
              <span className="inline-flex items-center gap-1 text-xs font-body px-2.5 py-1 rounded-full"
            ...
            <div className="mb-4">
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-xs font-body" style={{ color:'var(--text-muted)' }}>Progress</span>
                <span className="text-xs font-display font-bold" style={{ color:p.accent }}>{p.progress}%</span>
              </div>
              <div className="h-1.5 rounded-full w-full overflow-hidden"
                   style={{ background:'var(--bg-secondary)' }}>
                <motion.div
                  initial={{ width:0 }} animate={inView?{ width:`${p.progress}%` }:{}}
            ...
            <div className="flex flex-wrap gap-1.5">
              {p.tech.slice(0,4).map(t => (
                <span key={t} className="text-xs font-body px-2 py-0.5 rounded-md"
          ...
          <div className="absolute inset-0 pointer-events-none rounded-2xl"
               style={{ background:`radial-gradient(circle at 90% 10%, ${p.accent}08, transparent 60%)` }} />
        </motion.div>
      ))}
    </div>
  );
    ...
    <div ref={ref} className="grid md:grid-cols-2 gap-8 items-start">
      <motion.div initial={{ opacity:0, x:-24 }} animate={inView?{ opacity:1, x:0 }:{}} transition={{ duration:0.6 }}>
        <div className="rounded-2xl overflow-hidden mb-5"
          ...
          <div className="p-4">
            <p className="font-display font-bold" style={{ color:'var(--text-primary)' }}>{personal.name}</p>
            <p className="font-body text-sm" style={{ color:'var(--accent)' }}>{personal.role} · {personal.tagline}</p>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-3">
          {stats.map(s => (
            <div key={s.label} className="p-3 rounded-xl text-center"
              ...
              <p className="font-body text-xs mt-0.5" style={{ color:'var(--text-secondary)' }}>{s.label}</p>
            </div>
          ))}
        </div>
      </motion.div>
      <motion.div initial={{ opacity:0, x:24 }} animate={inView?{ opacity:1, x:0 }:{}} transition={{ duration:0.6, delay:0.1 }}>
        <p className="font-body text-sm leading-relaxed mb-6" style={{ color:'var(--text-secondary)' }}>
          {personal.bio} I completed my BCA from World College of Technology & Management and have 1+ year of web development experience.
        </p>
        ...
        <div className="space-y-2 mb-4">
          {education.map((e,i) => (
            <div key={i} className="flex items-start gap-3 p-3 rounded-xl"
              ...
              <div>
                <p className="font-display font-semibold text-xs" style={{ color:'var(--text-primary)' }}>{e.degree}</p>
                <p className="font-body text-xs" style={{ color:'var(--text-secondary)' }}>{e.institution}</p>
                <p className="font-body text-xs" style={{ color:'var(--accent)' }}>{e.year}</p>
              </div>
            </div>
          ))}
        </div>
        {achievements.map((a,i) => (
          <div key={i} className="flex items-start gap-3 p-4 rounded-xl glow-box"
               style={{ background:'var(--bg-card)', border:'1px solid rgba(99,102,241,0.2)' }}>
            <span className="text-2xl">{a.icon}</span>
            <div>
              <p className="font-display font-bold text-sm" style={{ color:'var(--text-primary)' }}>{a.title}</p>
              <p className="font-body text-xs mt-0.5" style={{ color:'var(--accent)' }}>{a.issuer} · {a.year}</p>
            </div>
          </div>
        ))}
      </motion.div>
